...
export const Balances = ({ members, expenses }) => {
  const balances = {};
  ...
  const getOwesList = () => {
    ...
    for (const m in balances) {
      ...
      if (val > 0) pos.push([m, val]);
      else if (val < 0) neg.push([m, -val]);
    }
    let i = 0,
      j = 0;
    while (i < neg.length && j < pos.length) {
      ...
      owes.push(`${debtor} owes ${creditor} $${settle.toFixed(2)}`);
      neg[i][1] -= settle;
      pos[j][1] -= settle;
      if (neg[i][1] === 0) i++;
      if (pos[j][1] === 0) j++;
